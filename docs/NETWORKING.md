...
Territory is stored in a 208×208 circular ownership grid. The complete grid is sent on join, then only when its revision changes. It is encoded as base64 `Uint16` data; `0` means unclaimed and `65535` means outside the arena.
